...
def meanSqrtV_3(k,v0,vbar,gamma):
    ...
    kbar = lambda t: 4.0*k*v0*np.exp(-k*t)/((gamma**2)*(1.0-np.exp(-k*t)))
    temp1 = lambda t: np.sqrt(2.0*c(t))*sp.gamma((1.0+delta)/2.0)/sp.gamma(delta/2.0)*sp.hyp1f1(-0.5,delta/2.0,-kbar(t)/2.0) # hyp1f1 confluent hyper-geometric function
    return temp1
